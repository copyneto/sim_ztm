CLASS zcltm_edoc_br_flex DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA gs_headerdata_cte TYPE j_1bcte_header .
    DATA:
...
CLASS ZCLTM_EDOC_BR_FLEX IMPLEMENTATION.
...
**Valida se encontrou algum registro de FRETE
  ...
  METHOD busca_tabela.
    ...
    CONSTANTS lc_erro_cte_na TYPE char1 VALUE '1'.
  ...
  METHOD cenario_transf.
    ...
    CONSTANTS: lc_typee   TYPE c VALUE 'E',
               ...
               lc_number8 TYPE c LENGTH 3 VALUE '008',
               lc_number9 TYPE c LENGTH 3 VALUE '009',
               lc_r       TYPE c VALUE 'R',
               lc_7       TYPE c VALUE '7'.
...
*           AND itmnum EQ @gc_10
     ...
     .
          IF sy-subrc IS INITIAL.
            ...
            lt_vbfa = VALUE #( FOR ls_mkpf IN lt_lin_mkpf (
                           vbeln = ls_mkpf-mblnr ) ).
            ...
            SELECT SINGLE docnum
              FROM j_1bnfe_active
              WHERE regio EQ @ls_nfe-regio
              AND nfyear EQ @ls_nfe-nfyear
              AND nfmonth EQ @ls_nfe-nfmonth
              AND stcd1 EQ @ls_nfe-stcd1
              AND model EQ @ls_nfe-model
              AND serie EQ @ls_nfe-serie
              AND nfnum9 EQ @ls_nfe-nfnum9
              AND docnum9 EQ @ls_nfe-docnum9
              AND cdv EQ @ls_nfe-cdv
              AND cancel EQ @space
              AND direct EQ 1
              INTO @DATA(lv_docnum_ref).

            SELECT SINGLE refkey
              FROM j_1bnflin
              WHERE docnum = @lv_docnum_ref
              INTO @DATA(lv_refkey).

            SELECT COUNT(*)
              FROM vbfa
              WHERE vbeln EQ @lv_refkey(10)
              AND mjahr EQ @lv_refkey+10(4)
              AND vbtyp_v EQ '7'
              AND vbtyp_n EQ 'R'.

            IF sy-subrc IS NOT INITIAL.
              "Documento de recebimento não encontrado para NFe referenciada
              rt_bapiret = VALUE #( BASE rt_bapiret ( type = lc_typee id = lc_id number = lc_number9 ) ).
              "Processo não mapeado, lançamento CT-e deve ser efetuado manualmente
              rt_bapiret = VALUE #( BASE rt_bapiret ( type = lc_typee id = lc_id number = lc_number8 ) ).
...
* Lógica para definir o processo REMESSA, TRANSFERÊNCIA OU VENDA
  ...
  METHOD preenche_bapi_cte.

    CONSTANTS: lc_xmlvers    TYPE j_1bnfexmlversion VALUE '3.00',
               lc_mode1      TYPE j_1bmodel VALUE 57,
               lc_cfop_1     TYPE j_1bcfop VALUE '1353AA',
               lc_cfop_2     TYPE j_1bcfop VALUE '2353AA',
               lc_icms       TYPE j_1btaxgrp VALUE 'ICMS',
               lc_interfvers TYPE n LENGTH 3 VALUE 001,
               lc_1          TYPE c LENGTH 1 VALUE '1',
               lc_2          TYPE c LENGTH 1 VALUE '2',
               lc_0          TYPE c LENGTH 1 VALUE '0',
               lc_btd_venda  TYPE c LENGTH 2 VALUE '80',
               lc_btd_transf TYPE c LENGTH 2 VALUE '18',
               lc_typee      TYPE c LENGTH 1 VALUE 'E',
               lc_id         TYPE c LENGTH 16 VALUE 'ZTM_EDOC_BR_FLEX',
               lc_number     TYPE c LENGTH 3 VALUE '007',
               lc_number11   TYPE c LENGTH 3 VALUE '011',
               lc_tfdt       TYPE lfart VALUE 'TFDT'.
    ...
    DATA(lv_cenario) = busca_tabela( EXPORTING iv_chave_cte = gv_chave_cte IMPORTING ev_erro = lv_erro ).
    ...
    CASE lv_cenario.
      WHEN gc_cenario-transferencia.
        ...
        IF gt_nfe_entrada[] IS NOT INITIAL.
          DATA(ls_nfe_ent) = gt_nfe_entrada[ 1 ].
        ENDIF.

        IF ls_nfe_ent IS NOT INITIAL.
          IF gv_bukrs IS INITIAL.
            gv_bukrs = ls_nfe_ent-bukrs.
          ENDIF.
        ENDIF.
        IF gt_ekko[] IS NOT INITIAL.
          gv_dzterm = gt_ekko[ 1 ]-zterm.
        ENDIF.
        gs_headerdata_cte = VALUE #(
                 interfvers   = lc_interfvers
                 direct_goods = lc_1
...
*                                 gross_amount = gv_vtprest
      ...
      WHEN gc_cenario-venda.
        ...
        lt_tm_itemdata = VALUE #( FOR ls_nfe_saida IN gt_nfe_saida
...
*                                 gross_amount = gv_vtprest
    ...
    REPLACE ALL OCCURRENCES OF '-' IN lv_dhrecbto WITH ' '.
    REPLACE ALL OCCURRENCES OF ':' IN lv_dhrecbto WITH ' '.

    gs_ctedoc-xmlvers = lc_xmlvers.
    ...
    gs_ctedoc-access_key-stcd1 = io_cte->retrieve_emit( )-cnpj.
    gs_ctedoc-access_key-model = lc_mode1.
    gs_ctedoc-access_key-serie = |{ lv_serie ALPHA = IN }|.
    gs_ctedoc-access_key-nfnum9 = |{ lv_nct ALPHA = IN }|.
    gs_ctedoc-access_key-docnum9 = |{ lv_chave_cte+34(09) ALPHA = IN }|.
    ...
    gs_ctedoc-authtime  = lv_dhrecbto+9(6).
    ...
    IF gs_ctedoc-regio_fr = gs_ctedoc-regio_to.
      ls_ctelin-cfop = lc_cfop_1.
    ELSE.
      ls_ctelin-cfop  = lc_cfop_2.
    ENDIF.

    IF gt_tm_itemdata[] IS NOT INITIAL.
      ls_ctelin-mwskz = gt_tm_itemdata[ 1 ]-tax_code.
    ELSE.
      rt_bapiret = VALUE #( BASE rt_bapiret ( type = lc_typee id = lc_id number = lc_number11 message_v1 = ls_nfe_ent-doc_frete ) ).
      RETURN.
    ENDIF.
    ...
    ls_ctestx-taxgrp = lc_icms.
    ls_ctestx-base   = gv_vtprest.
    ls_ctestx-taxval = io_cte->retrieve_infcte( )-imp-vtottrib.
    IF ls_ctestx-taxval IS INITIAL.
      ls_ctestx-taxval = io_cte->retrieve_infcte( )-imp-icms-icms00-vicms.
    ENDIF.
    IF ls_ctestx-base IS NOT INITIAL.
    ...
    IF lt_tm_itemdata IS NOT INITIAL.
      ...
      INNER JOIN /scmtms/d_torite ON parent_key = /scmtms/d_torrot~db_key
                                 AND /scmtms/d_torite~base_btd_id NE ''
      ...
      SHIFT lv_delivery_ref_of-base_btd_id LEFT DELETING LEADING lc_0.
      lv_vbeln = |{ lv_delivery_ref_of-base_btd_id ALPHA = IN }|.
    ...
    LOOP AT lt_infnfe ASSIGNING FIELD-SYMBOL(<fs_infnfe>).
      ...
      IF lv_exists = abap_true.
        gs_headerdata_cte-cnpj_servtaker = ls_cteref-stcd1.
      ENDIF.

      IF lv_delivery_ref_of-base_btd_id IS NOT INITIAL.
        IF lv_delivery_ref_of-base_btd_id(2) = lc_btd_transf.
          gs_headerdata_cte-direct_goods = lc_1.
        ELSE.
          gs_headerdata_cte-direct_goods = lc_2.
        ENDIF.
      ENDIF.

      ls_cteref-vbeln = lv_vbeln.
      APPEND ls_cteref TO gt_cteref.
    ENDLOOP.


    rt_bapiret =   executa_bapi_cte( io_cte ).
  ...
  METHOD executa_bapi.
    ...
    DATA(lv_task) = |{ gs_headerdata-ref_doc_no }| & |{ lv_ts }|.

    CALL FUNCTION 'ZFMTM_EDOC_BR_FLEX'
      STARTING NEW TASK lv_task
      CALLING task_finish ON END OF TASK
      EXPORTING
        is_headerdata       = gs_headerdata
        is_export           = ls_export_flex
      TABLES
        tt_return           = gt_return
...
*    bapiret = preenche_bapi( io_cte ).
    bapiret = preenche_bapi_cte( io_cte ).
    ...
    CASE lv_cenario.
      WHEN gc_cenario-transferencia.
        ...
        IF gt_ekko[] IS NOT INITIAL.
          gv_dzterm = gt_ekko[ 1 ]-zterm.
        ENDIF.
        gs_headerdata = VALUE #( invoice_ind  = abap_true
                                 doc_type     = gc_re
                                 doc_date     = sy-datum
                                 pstng_date   = sy-datum
*                                 ref_doc_no   = gv_chave_cte
                                 ref_doc_no   = lv_doc_ref
                                 comp_code    = gv_bukrs
                                 currency     = gc_cur
                                 gross_amount = gv_vtprest
*                                 pmnttrms     = gc_b030
...
*                                 pmnttrms     = gc_b030
  ...
  METHOD executa_bapi_cte.
    ...
    CALL FUNCTION 'ZFMTM_EDOC_BR_FLEX_V1'
      STARTING NEW TASK lv_task
      CALLING task_finish_cte ON END OF TASK
      EXPORTING
        is_headerdata       = gs_headerdata_cte
        is_ctedoc           = gs_ctedoc
      TABLES
        tt_cteref           = gt_cteref
        tt_ctestx           = gt_ctestx
    ...
    WAIT FOR ASYNCHRONOUS TASKS UNTIL gv_result IS NOT INITIAL.
    MOVE-CORRESPONDING gt_return TO rt_bapiret.
